...
from laya_coreml.snake.game import DIRECTIONS, SnakeGame, hamiltonian_cycle
from laya_coreml.snake.policy import LayaPolicy, local_checkpoint
...
def test_recording_rejects_invalid_wall_clock_timestamps(tmp_path, times):
    from laya_coreml.snake.replay import load_record

    path = tmp_path / "record.jsonl"
    events = [{"type": "metadata", "format": "laya-coreml-snake-v1"}]
...
def test_recording_preserves_actual_timestamps_and_probabilities(tmp_path):
    ...
    path = tmp_path / "record.jsonl"
    metadata = {"type": "metadata", "format": "laya-coreml-snake-v1"}
    frames = [
...
@pytest.mark.parametrize("filename", ["coreml-snake-showcase.jsonl"])
