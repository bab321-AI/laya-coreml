...
def play(argv=None):
    parser = argparse.ArgumentParser(
        prog="laya-coreml-snake",
        description=__doc__,
        epilog="Also: laya-coreml-snake benchmark --help | laya-coreml-snake export --help",
    )
    ...
    parser.add_argument(
        "--compute-units",
        choices=("cpu_ne", "cpu_gpu", "cpu", "all"),
        help="Default: CPU+ANE for ANE bundles, CPU+GPU for ordinary exports",
    )
    ...
    print("Loading local Core ML bundle; no network requests...", file=sys.stderr)
    policy = LayaPolicy(
        args.model,
