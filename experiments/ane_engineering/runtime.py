"""Research-only host lookup + one ANE graph + CPU action-head runtime."""
...
class ANEAgent(PromptMixin, ResultMixin):
    def __init__(self, source, package, *, length=96, compute_units="cpu_ne"):
        self.source = Path(source)
        self.manifest = verify_manifest(self.source, package, length=length)
        ...
        self.model = ct.models.MLModel(str(package), compute_units=units[compute_units])
        ...
        with safe_open(str(self.source / "model.safetensors"), framework="numpy") as weights:
